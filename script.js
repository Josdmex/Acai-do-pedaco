...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 4. Lógica de Arrastar com Mouse (PC) - Aqui o JS precisa trabalhar
    ...
    slider.addEventListener('mousemove', (e) => {
        ...
        const walk = (x - startX) * 1.5; // Multiplicador da velocidade do mouse
        slider.scrollLeft = scrollLeft - walk;
    });

    // 5. Lógica de Arrastar com Dedo (Celular) - A MÁGICA ESTÁ AQUI
    slider.addEventListener('touchstart', (e) => {
        isDown = true;
        isDragging = false;
        cancelAnimationFrame(animationId); // Só pausa o motor automático pra você arrastar
    }, {passive: true});

    slider.addEventListener('touchend', () => {
        isDown = false;
        playCarousel(); // Soltou o dedo, volta a girar
    });

    slider.addEventListener('touchmove', () => {
        if (!isDown) return;
        isDragging = true; // Só avisa que tá arrastando pra não virar o card sem querer
        // APAGAMOS a matemática daqui! Deixamos o seu iPhone/Android fazer o scroll nativo (que é 100% liso).
    }, {passive: true});
